...
public partial class NicoVideoHandler : ISiteHandler
{
    ...
    public Task<string> RewriteUrl(string url, Uri uri)
    {
        ...
        var (m, group) = new[]
        {
            (NicoID1.Match(url), 3), (NicoID2.Match(url), 2), (NicoID4.Match(url), 1)
    ...
    [GeneratedRegex(
        @"^(sm\d+|nm\d+|am\d+|fz\d+|ut\d+|dm\d+|so\d+|ax\d+|ca\d+|cd\d+|cw\d+|fx\d+|ig\d+|na\d+|om\d+|sd\d+|sk\d+|yk\d+|yo\d+|za\d+|zb\d+|zc\d+|zd\d+|ze\d+|nl\d+|ch\d+|\d+|lv\d+)$",
