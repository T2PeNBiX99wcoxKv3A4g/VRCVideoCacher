...
public partial class AutoStartShortcut : Singleton<AutoStartShortcut>
{
    ...
    public void TryUpdateShortcutPath2()
    {
        ...
        else
        {
            ...
            {
                Log.Warning(ex, "Failed to read shortcut from {Shortcut}, recreating...", shortcut);
                ...
                newShortcut.WriteToFile(shortcut);
            }
        }
    }
    ...
    public void CreateShortcut2()
    {
        ...
        var vrcxDir = Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "VRCX");
        if (!Directory.Exists(vrcxDir))
        {
            Log.Information("VRCX isn't installed");
            return;
        }

        var path = Path.Join(vrcxDir, "startup");
        if (!Directory.Exists(path))
            Directory.CreateDirectory(path);

        var useSteam = ShouldUseSteamShortcut();
        var shortcutPath = Path.Join(path,
            $"{ShortcutName}{(useSteam ? SteamShortcutExtension : ExeShortcutExtension)}");
    ...
    private void RemoveLegacyShortcut(bool createIfAnyFound)
    {
                     ...
                     shortCut.EndsWith(legacyExtension, StringComparison.OrdinalIgnoreCase)))
        {
            ...
            {
                Log.Warning(ex, "Failed to delete legacy shortcut: {ShortCut}", shortCut);
            }
        }
    ...
    private string? GetOurShortcut()
    {
        ...
        var targetExtension = ShouldUseSteamShortcut() ? SteamShortcutExtension : ExeShortcutExtension;
        var shortcuts = FindShortcutFiles(shortcutPath);
    ...
    private bool TryGetVrcxVersion(out string? version)
    {
        ...
        try
        {
            // Check Windows registry for installed applications (both HKCU and HKLM)
        ...
        // Try standard install paths as fallback
        var localAppDataPrograms = Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Programs", "VRCX");
        if (TryGetVrcxVersionFromFile(localAppDataPrograms, out version))
            return true;

        var programFiles = Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "VRCX");
        if (TryGetVrcxVersionFromFile(programFiles, out version))
            return true;
    ...
    private bool TryGetVrcxVersionFromFile(string? directory, out string? version)
    {
        ...
        var exePath = Path.Join(directory, "VRCX.exe");
        if (File.Exists(exePath))
        {
            ...
            {
                Log.Warning(ex, "Error getting VRCX version from exe: {ExePath}", exePath);
            }
        }
